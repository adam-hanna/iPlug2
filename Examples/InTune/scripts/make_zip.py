...
def  main():
  ...
  installer = "\installer\InTune Installer.exe"
   
  if demo:
    installer = "\installer\InTune Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\InTune manual.pdf" 
  ]
  ...
  ZIPNAME = "InTune-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "InTune-v" + FULLVERSIONSTR + "-win-demo.zip"
