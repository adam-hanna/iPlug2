...
static void DBGMSG4(const char* format, ...)
{
  char buf[4096], * p = buf;
  va_list args;
  int     n;
...
{
  DBGMSG4("EvaluateJavaScript; %s", scriptStr);
  if (mWebViewWnd)
  {
    DBGMSG4("has mWebViewWnd");
    ...
    mWebViewWnd->ExecuteScript(scriptWide, Callback<ICoreWebView2ExecuteScriptCompletedHandler>(
      [func](HRESULT errorCode, LPCWSTR resultObjectAsJson) -> HRESULT {
        DBGMSG4("errCode %d; result %s", errorCode, resultObjectAsJson);
        if (func && resultObjectAsJson) {
          WDL_String str;
          UTF16ToUTF8(str, resultObjectAsJson);
          DBGMSG4("running %s", str.Get());
          func(str.Get());
        }
        return S_OK;
      }).Get());
